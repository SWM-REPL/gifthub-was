...
public class KakaoService implements OAuth2Service {
	...
	public OAuthUserInfoDto getUserInfo(OAuthTokenDto oAuthTokenDto) {
		try {
			...
			JsonElement profile = responseElement.getAsJsonObject().get("kakao_account").getAsJsonObject().get("profile");
			if (profile != null) {
				email = profile.getAsJsonObject().get("email") != null
						? profile.getAsJsonObject().get("email").getAsString()
						: null;
				nickname = profile.getAsJsonObject().get("nickname") != null
	...
	public OAuth delete(User user) {
		OAuth oAuth = oAuthRepository.findByUserAndPlatform(user, OAuthPlatform.KAKAO).orElseThrow(
